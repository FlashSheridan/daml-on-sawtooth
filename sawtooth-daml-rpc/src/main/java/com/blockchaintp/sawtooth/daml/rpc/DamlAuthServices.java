...
public final class DamlAuthServices implements AuthService {
  ...
  private Key<String> authorizationKey;
  ...
  private com.daml.ledger.api.auth.Claims decodeAndParse(final io.grpc.Metadata headers)
    ...
    final Metadata.Key<String> authorizationKey = AUTHORIZATION_KEY();
  ...
  private Claims payloadToDAClaims(final AuthServiceJWTPayload payload) {
    ...
    final Claims claims =
        new Claims(claimsList.toList(), payload.ledgerId(), payload.participantId(), payload.exp());
    return claims;
  ...
  public Key<String> AUTHORIZATION_KEY() {
    // TODO this is a strange implementation requirement
    if (null == authorizationKey) {
      setAuthorizationKey(Metadata.Key.of("Authorization", Metadata.ASCII_STRING_MARSHALLER));
    }
    return authorizationKey;
  }

  private void setAuthorizationKey(Key<String> key) {
    synchronized (this) {
      authorizationKey = Metadata.Key.of("Authorization", Metadata.ASCII_STRING_MARSHALLER);
    }
  }

  @Override
  public void com$daml$ledger$api$auth$AuthService$_setter_$AUTHORIZATION_KEY_$eq(Key<String> key) {
    // TODO Part of the weird way the key is declaried as a val inside a trait
