...
public final class TimeKeeperRunnable implements Runnable {
  ...
  public TimeKeeperRunnable(final KeyManager kmgr, final Stream argStream) {
    ...
    this.recordAddress =
        Namespace.makeAddress(Namespace.getNameSpace(), this.keyManager.getPublicKeyInHex());
  }
  ...
  public void run() {
    ...
    Timestamp ts = Timestamp.newBuilder().setSeconds(instant.getEpochSecond())
        .setNanos(instant.getNano()).build();
    TimeKeeperUpdate update = TimeKeeperUpdate.newBuilder().setTimeUpdate(ts).build();

    List<String> inputAddresses =
        Arrays.asList(this.recordAddress, Namespace.TIMEKEEPER_GLOBAL_RECORD);
    List<String> outputAddresses =
        Arrays.asList(this.recordAddress, Namespace.TIMEKEEPER_GLOBAL_RECORD);
    Transaction updateTransaction = SawtoothClientUtils.makeSawtoothTransaction(this.keyManager,
        Namespace.TIMEKEEPER_FAMILY_NAME, Namespace.TIMEKEEPER_FAMILY_VERSION_1_0, inputAddresses,
        outputAddresses, Arrays.asList(), update.toByteString());

    Batch batch =
        SawtoothClientUtils.makeSawtoothBatch(this.keyManager, Arrays.asList(updateTransaction));

    try {
      LOGGER.info("Sending a participant time update {} time={}",
          this.keyManager.getPublicKeyInHex(), new Date(Timestamps.toMillis(ts)));
      sendBatch(batch);
